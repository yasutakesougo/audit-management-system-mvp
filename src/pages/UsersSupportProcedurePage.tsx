...
const alignmentChecklist = [
  '個別支援計画で定義した目標・支援ステップを、支援手順の実施テンプレートに自動反映すること。',
...
const trainingModules = [
  ...
  'デイリー運用トレーニング: 行動観察から記録作成までをロールプレイで反復し、タグ設定の基準を統一。',
      ...
      <Paper elevation={2} sx={{ p: { xs: 3, md: 4 } }}>
        <Stack spacing={2}>
          ...
          <Alert severity="info" variant="outlined">
            の支援現場における「支援手順の実施」の位置づけを整理し、既存の事業所時間割記録・健康記録との連携観点をまとめたレポートです。入力機能の実装に先立ち、チームで共有すべき情報構造と運用ルールを可視化します。
          </Alert>
          ...
          <Typography variant="body1">
            支援手順の実施は、個別支援計画で定義された支援方法と日々の行動記録を一体化し、スタッフが同じシナリオを共有できるようにする業務基盤です。本レポートでは「誰が・いつ・どの情報を更新するか」というワークフローを整理し、段階的なシステム整備のロードマップを提示します。
          </Typography>
        </Stack>
      </Paper>
      ...
      <Paper elevation={1} sx={{ p: { xs: 3, md: 4 } }}>
        <Stack spacing={3}>
          <Box>
            ...
            <Typography variant="body2" color="text.secondary">
              支援手順の実施が解決すべき現場課題を三つの代表的なシナリオで整理し、優先する成果指標を明確化します。
            </Typography>
          </Box>
