...
export const HydrationHud: React.FC = () => {
  ...
  const [breakerSnapshots, setBreakerSnapshots] = useState(() => getBreakerSnapshots());

  useEffect(() => {
    if (!hudEnabled) return undefined;
    const cleanup = initBreakerStore();
    return cleanup;
  }, [hudEnabled]);

  useEffect(() => {
    if (!hudEnabled) return undefined;
    const unsubscribe = subscribeBreakerSnapshots(setBreakerSnapshots);
    return unsubscribe;
  }, [hudEnabled]);
      ...
      {visible ? (
        <div data-testid="prefetch-hud" style={panelStyle}>
          <div style={panelContentStyle}>
            ...
            {/* ── Fetch Health (Shadow Breaker) ───────────────── */}
            <FetchHealthSection snapshots={breakerSnapshots} />
