...
import { env, getAppConfig, isE2E, isE2eMsalMockEnabled, isDemoModeEnabled, shouldSkipLogin, shouldSkipSharePoint } from '@/lib/env';
...
const AppShell: React.FC<{ children: React.ReactNode }> = ({ children }) => {
  ...
  const groupedNavItems = useMemo(() => {
    return groupNavItems(filteredNavItems);
  }, [filteredNavItems]);
...
const ConnectionStatus: React.FC = () => {
  ...
  const e2eMode = isE2E() && !isVitest;
  const sharePointDisabled = shouldSkipSharePoint();
...
const FooterQuickActions: React.FC<{ fixed?: boolean }> = ({ fixed = true }) => {
  ...
  const footerTestIds: Record<string, string> = {
    'schedules-month': TESTIDS.footer.schedulesFooterMonth,
