...
const BASE_TIME_SHIFT_CANDIDATES: TimeShiftCandidate[] = [
  { id: 'shift-minus-30', label: '30分早める', offsetMinutes: -30 },
  { id: 'shift-plus-30', label: '30分遅らせる', offsetMinutes: 30 },
  { id: 'shift-plus-60', label: '60分遅らせる', offsetMinutes: 60 },
];
...
export const ScheduleConflictGuideDialog: React.FC<Props> = ({
  ...
  // 生活支援同士の重複があり、対象スケジュールが生活支援の場合にアクションボタンを表示
  const canApplyTimeShift = useMemo(() => {
    if (!schedule || !onApplySuggestion) return false;

    // 生活支援同士の重複があるかチェック
    const hasLifeSupportConflict = conflicts.some(
      (c) => c.kind === 'user-life-support-vs-support'
    );

    // 対象スケジュールが生活支援カテゴリかチェック（BaseScheduleの構造に合わせて調整）
    const isLifeSupport = schedule.category === 'User' && (
      schedule.title?.includes('生活支援') ||
      schedule.title?.includes('一時ケア') ||
      schedule.title?.includes('ショートステイ')
    );

    return hasLifeSupportConflict && isLifeSupport;
  }, [schedule, conflicts, onApplySuggestion]);

  // 職員代替案が表示可能かチェック
  const canShowStaffAlternatives = useMemo(() => {
    if (!schedule || !onApplySuggestion || !allSchedules.length) return false;

    // 職員関連の衝突があるかチェック
    const hasStaffConflict = conflicts.some(
      (c) => c.kind === 'staff-life-support-vs-staff'
    );

    // 対象スケジュールが職員を持つカテゴリかチェック
    const hasStaff = schedule.category === 'User' || schedule.category === 'Staff';

    return hasStaffConflict && hasStaff;
  }, [schedule, conflicts, onApplySuggestion, allSchedules]);
  ...
  return (
    <Dialog
      ...
      maxWidth="sm"
      fullWidth
      data-testid="schedule-conflict-guide-dialog"
    >
      <DialogTitle
        data-testid="schedule-conflict-guide-title"
      >
        ⚠️ スケジュールの重複について
      ...
      <DialogContent dividers data-testid="schedule-conflict-guide-content">
        {guideItems.length === 0 ? (
        ...
        {canApplyTimeShift && timeShiftCandidates.length > 0 && (
          <Box sx={{ mt: 2, p: 2, bgcolor: 'primary.50', borderRadius: 1 }}>
            ...
            <Stack
              ...
              spacing={1}
              flexWrap="wrap"
              data-testid="schedule-conflict-guide-suggestion-buttons"
            >
              {timeShiftCandidates.map((candidate) => (
                <Button
                  ...
                  {...(candidate.offsetMinutes === 30
                    ? { 'data-testid': 'schedule-conflict-guide-apply-30min-later' }
                    : candidate.offsetMinutes < 0
                    ? { 'data-testid': 'schedule-conflict-guide-apply-30min-earlier' }
                    : candidate.offsetMinutes === 60
                    ? { 'data-testid': 'schedule-conflict-guide-apply-60min-later' }
                    : {})}
                >
                  {candidate.label}
                </Button>
        ...
        {canShowStaffAlternatives && staffAlternatives.length > 0 && (
          <Box sx={{ mt: 2, p: 2, bgcolor: 'success.50', borderRadius: 1 }}>
            ...
            <Stack
              direction="column"
              spacing={1}
              data-testid="schedule-conflict-guide-staff-alternatives"
            >
              {staffAlternatives.map((alternative, index) => (
                ...
                >
                  <Box sx={{ flex: 1 }}>
                    ...
                    {alternative.skillsMatched.length > 0 && (
                      <Box sx={{ mt: 0.5 }}>
        ...
        {canShowVehicleAlternatives && vehicleAlternatives.length > 0 && (
          <Box sx={{ mt: 2, p: 2, bgcolor: 'warning.50', borderRadius: 1 }}>
            ...
            <Stack
              direction="column"
              spacing={1}
              data-testid="schedule-conflict-guide-vehicle-alternatives"
            >
        ...
        {canShowRoomAlternatives && roomAlternatives.length > 0 && (
          <Box sx={{ mt: 2, p: 2, bgcolor: 'secondary.50', borderRadius: 1 }}>
            ...
            <Stack
              direction="column"
              spacing={1}
              data-testid="schedule-conflict-guide-room-alternatives"
            >
        ...
        {canShowEquipmentAlternatives && equipmentAlternatives.length > 0 && (
          <Box sx={{ mt: 2, p: 2, bgcolor: 'info.50', borderRadius: 1 }}>
            ...
            <Stack
              direction="column"
              spacing={1}
              data-testid="schedule-conflict-guide-equipment-alternatives"
            >
      ...
      <DialogActions data-testid="schedule-conflict-guide-actions">
        <Button
          onClick={onClose}
          variant="text"
          data-testid="schedule-conflict-guide-close"
        >
          閉じる
        </Button>
      </DialogActions>
...
export function toGuideItem(conflict: ScheduleConflict): GuideItem {
  switch (conflict.kind) {
    case 'vehicle-double-booking':
      return {
        kind: conflict.kind,
        title: '車両のダブルブッキングが発生しています',
        description:
          '同一車両が複数の予定で同時間帯に使用されています。',
        suggestions: [
          '時間帯をずらして重複を解消してください。',
          '他の車両への変更を検討してください。',
          '送迎計画を見直してください。',
    ...
    case 'room-double-booking':
      return {
        kind: conflict.kind,
        title: '部屋のダブルブッキングが発生しています',
        description:
          '同一部屋が複数の予定で同時間帯に使用されています。',
        suggestions: [
          '時間帯をずらして重複を解消してください。',
          '他の部屋への変更を検討してください。',
          '会議室の利用計画を見直してください。',
    ...
    case 'equipment-conflict':
      return {
        kind: conflict.kind,
        title: '設備の利用衝突が発生しています',
        description:
          '同一設備が複数の予定で同時間帯に使用されています。',
        suggestions: [
          '時間帯をずらして重複を解消してください。',
          '他の設備への変更を検討してください。',
          '設備の利用計画を見直してください。',
    ...
    case 'org-resource-conflict':
      return {
        kind: conflict.kind,
        title: '組織リソースの衝突が発生しています',
        description:
          '組織のリソースに関して衝突が発生しています。',
        suggestions: [
          'リソースの優先度を確認してください。',
          '時間帯の調整を検討してください。',
          '代替リソースの利用を検討してください。',
    ...
    case 'transportation-overlap':
      return {
        kind: conflict.kind,
        title: '送迎予定の重複が発生しています',
        description:
          '送迎計画において時間帯の重複が発生しています。',
        suggestions: [
          '送迎時間を調整してください。',
          '送迎ルートを見直してください。',
          '車両の配車計画を調整してください。',
