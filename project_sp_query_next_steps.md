...
3.  [ ] **Phase 3: 残存ターゲットの系統的スキャンと移行** (Next)
    - [x] `src/features/daily/infra/DataProviderDailyRecordRepository.ts` (Canonical load/list)
    - [x] `src/features/users/infra/DataProviderUserRepository.ts` (ACCESSORY_LIST_JOIN_FIELD / isActive)
    - [x] `src/infra/sharepoint/repos/schedulesRepo.ts` (EventDate range / personId)
    - [x] `src/features/meeting-minutes/infra/DataProviderMeetingMinutesRepository.ts` (category/published/date-range)
