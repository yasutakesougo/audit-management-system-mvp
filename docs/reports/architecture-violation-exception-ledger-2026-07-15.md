...
- ベースコミット: `ab7b46bfaf16061dbd743560e5075e3008930510`
- 計測設定:
...
- 件数の扱い（固定方針）:
- **918件**: 現行CI (`npm run arch:check`) が `--ignore-known` 前提で管理している既知違反数
- **922件**: `--ignore-known` を外した生の再集計件数（`depcruise --no-ignore` 相当）
- **差分4件**: 未分類または整合待ちの差分として扱い、原因精査前に `918件` を確定値として扱う
- 結果:
  - `npm run arch:check`: **no new violations**, 918 known ignored（管理上の既知違反ベース）
  - `depcruise --no-ignore`: **922 violations**（生の依存違反数）
...
| ルール | 件数 |
| --- | ---: |
| no-external-feature-internals | 460 |
| no-cross-feature-internals | 413 |
...
- `npm run arch:check` の既知件数が `918` に揺れうるため（既知項目の再点検結果）、過去PRとの比較をし、再発生なしなら除外再評価を追加。
