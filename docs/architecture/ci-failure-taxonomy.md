...
初回作成時の参考調査日: 2026年6月19日
初回作成時の参考コミット: `7dde895340c5a6914f2c12ecd858621d5821a105`

## 1. 調査目的
本ドキュメントは、Audit Management System MVP におけるプルリクエスト（PR）トリアージ業務の標準化を目的とします。
本プロジェクトでは、小さな変更やドキュメントのみの変更（docs-only PR）であっても多くの CI チェックが自動実行されます。その中で発生するテストや型チェックの失敗を効率的に分類・判断し、開発者が「直すべき失敗（Diff-Caused）」か「待つべき失敗（Non-Diff-Caused / Flaky / External）」かを素早く判別できるトリアージランブックを定義します。
...
3. **Action or Wait（行動と待機の選択）**:
   - 自分の変更が原因であれば追加コミットで修正し、外部要因や flaky によるものであれば required checks と `mergeStateStatus` を確認した上で Ready 化を検討する。
...
* **チェック項目**:
  ...
  - 確認中は Draft 状態を維持し、Ready 化前に required checks と `mergeStateStatus` を確認する。

---

## 4. CI Check 種別と失敗分類
主要な CI チェックと、その役割を以下に整理します。チェック名は GitHub Actions の更新により変わる可能性があるため、PRごとに `gh pr checks <PR番号>` の実行結果を優先します。
...
graph TD
    ...
    G -- NO --> I[Ready化を検討 (無関係な失敗として分類)]
```

---

## 6. Docs-only PR の扱い
ドキュメントのみを修正する PR は、スコープと CI 失敗原因を分離しやすい代表例です。各 CI チェックで発生し得る失敗の分類と対応方針を以下に整理します。
...
| **`quality_extended` / `csp`** | 原則なし | **要ログ確認** | まず失敗ログを確認し、Diff-Caused / External / Flaky に分類する。差分由来でなければ追加修正なし。 |
...
### 2. 対応手順
* **docs-only PR の場合**: required checks であっても、明らかに自分の変更と無関係であれば rerun を1回試行し、それでも通らない場合は「既存の Flaky / External」として分類した上で、管理者やレビュー担当者に状況を報告し、Ready 化は required checks と `mergeStateStatus` を確認して判断します。
...
## 9. Ready Candidate 判定
PR を「Ready for review」へ移行し、レビュー可能と判断するためのチェックリストです。
...
### 2. 状態による移行判断
* **Ready 化を検討してよい（Candidate）**:
  - 上記チェックリストをすべてクリアしている状態。
* **追加修正が必要なため Ready 化不可**:
