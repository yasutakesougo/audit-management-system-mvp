...
| `#2263` `test(kiosk-toilet): cover correction error UI boundary` | correction error UI boundary | `src/features/kiosk/toilet/__tests__/ToiletDailyBoard.spec.tsx` |
| `#2264` `test(kiosk-toilet): cover correction success refresh behavior` | success refresh behavior | `src/features/kiosk/toilet/__tests__/useToiletRecords.spec.ts` |
...
- 訂正保存失敗後、保存ボタンを再び enabled にする。
- 訂正保存失敗後、キャンセルボタンも再び enabled にする。

対象テスト:

- `ToiletDailyBoard` correction error tests

## Success Refresh Boundary

訂正保存成功時は、repository update の戻り値を呼び出し元に返し、選択日の records を再取得して画面状態が訂正済みデータへ追従できる状態として固定している。

固定済みの期待:

- `correct()` は repository update が返した訂正済み record を返す。
- 訂正保存成功後、選択日の records を再取得する。
- 再取得後の `records` は訂正済みデータへ追従する。
- 訂正保存成功後、`error` state を残さない。

対象テスト:

- `useToiletRecords` correction success refresh tests
...
- Correction reason / audit history を追加する前の test-only 境界
- SharePoint 接続を伴わない correction history projection の純粋関数テスト

ただし、現時点で訂正機能の中核安全境界は repository、dialog cancel、save-in-flight UI、error UI、success refresh の 5 層で固定済みである。
