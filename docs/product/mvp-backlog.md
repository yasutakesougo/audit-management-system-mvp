...
[イソカツシステム完成形ブループリント](isokatsu-completion-blueprint.md) では、最初の完成ラインを **Phase 1: 現場導入できる完成** と定義する。本バックログの P0 は、この Phase 1 を成立させるための最小実装群である。

特に優先する起点は `/today` と `/users/:userId` である。`ActionQueueCard`、UserDetailPage ハブ化、DailyRecord の構造化タグ・次遷移、ContextPanel、ExceptionTable は、紙の代替として「今日の仕事」「利用者ごとの文脈」「未記録・未完了」を見える化するための P0 として扱う。

請求・AI レビュー・PDCA 高度化は重要だが、Phase 1 のブロッカーにはしない。これらは、Phase 2「支援記録 OS」、Phase 3「管理・請求・監査」の後続バックログとして分離する。
...
| 完成ライン | 対象 MVP | 扱い |
|------------|----------|------|
| Phase 1: 現場導入できる完成 | MVP-001 から MVP-007 | `/today` と `/users/:userId` を中心に、現場導線・未記録見える化・例外把握を優先する |
| Phase 2: 支援記録 OS としての完成 | MVP-012, MVP-013, MVP-017, MVP-020 | 支援計画、記録、モニタリング、PDCA、AI レビューを根拠の流れとして接続する |
| Phase 3: 管理・請求・監査の完成 | MVP-010, MVP-011, MVP-014, MVP-018, MVP-019 と請求系後続 Issue | 月次、請求、修正履歴、監査説明を管理者向けに固める |
| Phase 4: 属人化しない運用完成 | MVP-008, MVP-015 と Runbook / Nightly Patrol / PR 運用の後続 Issue | SharePoint、権限、保存失敗、CI、運用手順を固定する |
...
| 日付 | バージョン | 変更内容 | 著者 |
|------|-----------|---------|------|
| 2026-07-09 | 1.1 | イソカツ Phase 1 との関係と完成ライン別の読み替えを追加 | Codex |
