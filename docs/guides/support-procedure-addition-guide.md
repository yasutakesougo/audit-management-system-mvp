...
## 🆔 本番IDエイリアスマップ (2026/05 現在)

今後の実装・不具合調査の基準として、以下のマッピングを維持してください。

| 利用者氏名 | DEMO Id | SP Item ID | 本番 UserID | alias (fixture等) | 状態 |
|:---|:---:|:---:|:---:|:---|:---|
| **石渡 由喜子** | `4` | `6` | **`I005`** | `U-002`, `U-003` | 済 |
| **桂川 進太朗** | `3` | `10` | **`I009`** | `U-001` | 済 |
| **中村 裕樹** | `7` | `23` | **`I022`** | `U-006`, `I017` | 済 |
| **塩田 裕貴** | (未定) | `16` | **`I016`** | `I016` | 済 (PR #1819 以降) |

### ⚠️ 今後の運用ルール

1. **UserID の優先順位**: 本番 SharePoint の `Users_Master` リストにある `UserID` (例: `I005`) を必ず確認し、最優先でエイリアスに含めること。
2. **Kiosk 用数値 ID**: `/kiosk/users/:id` で使われる数値 ID（SharePoint Item ID や DEMO Id）も `alias` に含め、画面遷移時の整合性を保つこと。
3. **Fixture 互換性**: テスト用の fixture `userId` が本番と異なる場合は、必ず alias に追加すること。
4. **実装のタイミング**: 静的マスタに 17 行データを実際に追加したタイミングで、対応する `isXxxUserId` 関数を作成・更新すること。未追加利用者の alias 関数だけを先に作らない。
5. **判定ロジックの分離**: 利用者ごとに `isXxxUserId` 関数を独立させ、`findUserProcedureDetail` 等でそれらを組み合わせて判定する現在のパターンを維持すること。
