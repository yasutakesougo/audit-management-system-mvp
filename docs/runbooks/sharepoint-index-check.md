...
## 🚨 緊急診断レポート (2026-04-16 観測結果)

実テナントの診断テスト（`diagnose:threshold`）により、以下の破綻状態が確認されています。

| リスト名 | アイテム数 | 状態 | アクション |
|---|---|---|---|
| **DriftEventsLog** (無印) | **37,413** | 🔥 **しきい値死** | アーカイブ/退役推奨。インデックスがないため API 拒否。 |
| **DriftEventsLog_v2** | **3,370** | ✅ **健全 (要維持)** | **現在の正系推奨。** `Detected_At` のインデックスは確認済み。 |
| **schedule_events** | **6** | 🟡 **注意** | 件数は少ないが、`EndDate` が未インデックス。予防的に追加が必要。 |

---

## 🚨 緊急診断レポート (2026-04-16 観測)
...
|---|---|---|---|
| **DriftEventsLog** (無印) | **37,413** | 🔥 **しきい値死** | アーカイブ/退役推奨。インデックスがないため API 拒否。 |
| **DriftEventsLog_v2** | **3,370** | ✅ **健全 (要維持)** | **現在の正系参照先。** `Detected_At` のインデックスは確認済み。 |
| **schedule_events** | **6** | 🟡 **注意** | 件数は少ないが、`EndDate` が未インデックス。予防的に追加が必要。 |
