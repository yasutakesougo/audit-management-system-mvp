# Schedule Conflict Detection - Testing Strategy

## Overview 🧪

スケジュール重複検知システムの包括的なテスト戦略。ユニットテストからE2Eテストまで、多層的なテストアプローチで仕様をロックし、将来の変更から保護します。

## Test Architecture

```
┌─ Unit Tests (Vitest) ──────────────────────────────────────────┐
│  conflictChecker.spec.ts                                        │
│  • hasTimeOverlap() - 時間重複ロジック                           │
│  • checkScheduleConflicts() - 競合検知メインロジック              │
│  • getConflictSeverity() - 重要度判定                           │
│  • Complex Scenarios - エッジケース・パフォーマンス               │
└─────────────────────────────────────────────────────────────────┘

┌─ E2E Tests (Playwright) ───────────────────────────────────────┐
│  schedule-conflicts.spec.ts                                     │
│  • UI Integration - 重複表示・警告インジケーター                 │
│  • User Workflow - 作成フロー・エラーハンドリング                │
│  • Cross-View Consistency - Week/Month/Day表示での一貫性        │
│  • Conflict Resolution - 重複解決ガイダンス                     │
└─────────────────────────────────────────────────────────────────┘
```

## Unit Test Coverage

> 注記: legacy の conflictChecker 実装は削除済み。現行は schedules adapter での競合検知（ETag/412）を中心にテストする。

### ⚡ Core Logic Tests
- **時間重複検知 (`hasTimeOverlap`)**
  - 部分重複 / 完全重複 / 包含関係
  - 隣接時間（`end === start`）→ 重複なし
  - 無効な日付の処理
  - タイムゾーン対応

- **競合検知 (`checkScheduleConflicts`)**
  - ダブルブッキング（同一職員）
  - 利用者予定重複
  - 職員不在
  - 編集時の自己除外
  - 必須フィールド不足時の処理

### 🔬 Edge Cases & Performance
- **境界値テスト**
  - 1分重複 / 完全同時間
  - 日をまたぐスケジュール
  - タイムゾーン混在

- **パフォーマンステスト**
  - 1000件の予定データで100ms以内処理
  - メモリリーク防止

## E2E Test Scenarios

### 📱 User Experience
- **視覚的重複表示**
  - 重複予定の警告アイコン・色分け
  - Week/Month/Day全表示での一貫性
  - ツールチップによる詳細情報

- **作成・編集フロー**
  - 重複エラーの即座検知
  - 保存防止 vs 警告付き保存
  - 解決ガイダンスの提供

### 🛠 Workflow Testing
- **重複解決プロセス**
  - 時間調整の自動提案
  - 既存予定編集オプション
  - 強制保存（管理者権限）

## Test Data Strategy

### 🎭 Test Fixtures
```typescript
// 基本パターン
const overlappingSchedules = [
  { start: '09:00', end: '10:00', type: 'lifeCare' },
  { start: '09:30', end: '10:30', type: 'lifeSupport' }
];

// エッジケース
const edgeCases = [
  { start: '23:00', end: '01:00+1' }, // 日またぎ
  { start: '09:00:00', end: '09:00:01' }, // 1秒差
];

// パフォーマンス
const largeDataset = generateSchedules(1000);
```

## Running Tests

### Unit Tests
```bash
# 単体実行
npx vitest run tests/unit/schedules.sharePointAdapter.conflict.spec.ts

# ウォッチモード
npx vitest watch schedules.sharePointAdapter.conflict

# カバレッジ
npx vitest run --coverage
```

### E2E Tests
```bash
# 重複検知E2Eテスト
npx playwright test schedule-conflicts

# ヘッドレスモードで実行
npx playwright test --headed schedule-conflicts

# 特定ブラウザのみ
npx playwright test --project=chrome schedule-conflicts
```

## Future Enhancements

### 📈 Test Expansion
- **Integration Tests**
  - SharePoint API連携テスト
  - リアルタイム更新テスト
  - 複数ユーザー同時編集

- **Visual Regression**
  - 重複表示UIのスクリーンショット比較
  - レスポンシブデザインテスト

### 🚀 Advanced Scenarios
- **Business Logic**
  - 利用者定員制限
  - 職員稼働時間制限
  - 施設リソース競合

- **Performance**
  - 大規模データでのスケーラビリティ
  - リアルタイム検知のレスポンス時間

## Test Maintenance

### 🔄 継続的改善
- **仕様変更への対応**
  - 新しい重複ルール追加時のテスト追加
  - API変更時の mock データ更新

- **データ品質保証**
  - 本番データパターンの反映
  - エラーケースの追加発見と対応

### ⚙️ CI/CD Integration
```yaml
# GitHub Actions例
- name: Run Unit Tests
  run: npm run test:unit -- conflictChecker

- name: Run E2E Tests
  run: npm run test:e2e -- schedule-conflicts

- name: Upload Coverage
  run: npm run coverage:upload
```

このテスト戦略により、重複検知システムの信頼性を確保し、将来の機能拡張に対する安全な基盤を提供します。