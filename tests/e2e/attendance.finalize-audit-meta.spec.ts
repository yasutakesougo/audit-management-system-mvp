...
test.describe('attendance finalize audit metadata', () => {
  test.use({
    baseURL: process.env.E2E_BASE_URL ?? 'http://127.0.0.1:5173',
  });

  test('shows finalized badge and optional audit metadata', async ({ page }) => {
    const strictAuditAssert = process.env.VITE_E2E_HAS_FINALIZE_AUDIT_FIELDS === '1';
    ...
    const badge = page.getByTestId('staff-attendance-finalized-badge');
    await expect(badge).toBeVisible();

    if (strictAuditAssert) {
      await expect(badge).toContainText(/確定済み（.+）/);
    } else {
      await expect(badge).toContainText('確定済み');
    }
  });
