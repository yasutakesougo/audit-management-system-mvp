...
test.describe('reception attendance unfinalize action guard e2e', () => {
  test.use({
  ...
  test('reception can unfinalize attendance day', async ({ page }) => {
    ...
    const unfinalizeButton = page.getByTestId('staff-attendance-unfinalize-btn');

    await expect(finalizeButton).toBeEnabled();
    await expect(unfinalizeButton).toBeDisabled();

    await finalizeButton.click();
    await expect(page.getByTestId('staff-attendance-finalized-badge')).toBeVisible();
    await expect(unfinalizeButton).toBeEnabled();

    await unfinalizeButton.click();
    await expect(page.getByTestId('staff-attendance-finalized-badge')).toHaveCount(0);
    await expect(finalizeButton).toBeEnabled();
  });

  test('admin can unfinalize attendance day', async ({ page }) => {
