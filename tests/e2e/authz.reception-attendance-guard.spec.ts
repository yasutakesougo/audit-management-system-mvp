...
const expectAttendancePageVisible = async (page: Page) => {
  const inputRoot = page.getByTestId('staff-attendance-input-root');
  const emptyState = page.getByTestId('staff-attendance-empty-state');
  await expect(inputRoot.or(emptyState).first()).toBeVisible();
};

test.describe('reception attendance guard e2e', () => {
  test.use({
  ...
  test('reception can access staff attendance route', async ({ page }) => {
    await bootstrapRole(page, 'reception', '/staff/attendance');

    await expectAttendancePageVisible(page);
    await expect(page.getByRole('heading', { name: accessDeniedHeading })).toHaveCount(0);
  });

  test('admin can access staff attendance route', async ({ page }) => {
