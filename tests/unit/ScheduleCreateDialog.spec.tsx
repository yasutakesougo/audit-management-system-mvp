...
describe('toCreateScheduleInput', () => {
  ...
  it('defaults non-user schedules to other and preserves explicit values', () => {
    const result = toCreateScheduleInput(
      buildForm({
        category: 'Org',
        endLocal: '',
    ...
    expect(result.endLocal).toBe('2025-11-12T11:00');
    expect(result.serviceType).toBe('other');

    const explicit = toCreateScheduleInput(
      buildForm({
        endLocal: '2025-11-12T12:30',
        serviceType: 'absence',
      })
    );
    expect(explicit.endLocal).toBe('2025-11-12T12:30');
    expect(explicit.serviceType).toBe('absence');
  });
