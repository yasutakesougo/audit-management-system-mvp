...
describe('mergeById', () => {
  it('Fetched より LocalDraft が優先される', () => {
    const fetched: Array<{ id: string; status: string; title: string }> = [{ id: 'a', status: '確定', title: 'x' }];
    const drafts: Array<{ id: string; status: string; title: string }> = [{ id: 'a', status: '遅刻', title: 'x' }];
