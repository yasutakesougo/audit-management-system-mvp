...
function Connect-SharePoint {
    ...
    if ((Test-Path $CertificatePath) -and $CertificatePassword) {
        Connect-PnPOnline -Url $SiteUrl -ClientId $ClientId -Tenant $TenantId -CertificatePath $CertificatePath -CertificatePassword $CertificatePassword
        return
    }
...
# Compliance_Checklist は存在しない場合もあるため、存在確認後に追加
try {
