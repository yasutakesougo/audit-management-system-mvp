...
for (const file of allTsFiles) {
  ...
  for (const key of keys) {
    ...
    // Replace in vi.mock and vi.doMock
    const reMock = new RegExp(`vi\\.m(ock|oMock)\\(\\s*(['"\`])${key.replace(/\//g, '\\/')}`, 'g');
    content = content.replace(reMock, `vi.m$1($2${val}`);
    
    // Also internal strings that match exact paths if used dynamically
    // But maybe let's just stick to vi.mock for now.
    // Also jest.mock if used
    const reJest = new RegExp(`jest\\.mock\\(\\s*(['"\`])${key.replace(/\//g, '\\/')}`, 'g');
    content = content.replace(reJest, `jest.mock($1${val}`);
    
    // Fix relative paths inside src/features/daily hooks and tests
    // e.g. vi.mock('../../hooks/useTableDailyRecordForm')
    // This is harder to do safely without parsing, but it's likely they used absolute imports `@/features/daily/...`
  }
...
console.log(`Replaced mock paths in ${replacedFiles} files.`);
